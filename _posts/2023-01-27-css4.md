...
.top-menu {
    padding: 31px 0;
    /* 콘텐츠 박스(ul)가 커질 때 같이 커지길 바란다면 padding */
    /* 콘텐츠 박스(ul)의 크기에 상관없이 자신의 크기를 가진다면 width */
...
.top-menu li>img {
    width: 100%;
    height: 100%;
    /* 부모의 크기가 바뀔 수 있으므로 %(비율)로 크기지정 / CSS 3.0 */
    object-fit: cover;
    /* html에 있는 이미지에 background-size스타일을 주는 방법 / 이미지의 크기와 높이를 정해준 뒤 사용해야 한다*/
