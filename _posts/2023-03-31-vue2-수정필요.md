...
<!-- thymeleaf의 each문과 Vue의 for문을 같이 쓰게되면 어떻게 될까? each문으로 9번 반복하고 for문으로 2번반복한다면 18개가 출력된다 -->
<!-- thymeleaf가 해석할 수 없는 v-for 9번 반복시키게 되고 Vue가 v-for를 2번 반복시키기 때문에 18개가 된것 -->
<!-- thymeleaf가 서버에서 페이지를 만들면 Vue가 클라이언트에서 페이지를 다시 만든다 -->
<!-- thymeleaf 혹은 Vue 둘 중에 하나만 써야한다 -->

XHR Fetch는 브라우저가 제공하는 것
