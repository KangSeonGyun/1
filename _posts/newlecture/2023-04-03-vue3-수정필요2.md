...
사용자는 index에 왔을 때 모든 page를 받게되며 page 전환이 부드러운 경험을 할 수 있다.   
SPA는 API서버로부터 데이터만 비동기적으로 로드하게 되고 js로 데이터만 바꿔끼게 된다.   
이제 SSR 방식처럼 page를 새로만드는 것이 아닌 데이터만 바꿔끼게 되므로 사용자는 page가 새로고침 됐다고 느낄 수 없다.   
