...
<form action ="signup"> <!--action 서버에서 찾을 프로그램-->
    	...
    	<!--
    	value 는 버튼에 보이는 이름																			
    	input type submit을 클릭하면 입력값을 서버에 전달가능.
    	-->
    	...
    	<!--
    		html5에서 추가된 태그들. 브라우저별로 ui가 달라져서 잘 안씀
    		라이브러리를 사용하는것을 권장 
    	-->
    	<label>가입일</label>
    	<input type="date">
...
mdn javascript 정규표현식 URL: 정규 표현식 - JavaScript
...
Entity
mdn Entity URL: Entity - MDN Web Docs Glossary: Definitions of Web-related terms
...
50강 Visual Studio Code Emmet snippets URL: Emmet Documentation
